...
public class TeacherController {
    ...
    public String dashboard(Authentication auth, Model model) {
        ...
        long classroomCount = classroomRepository.countByTeacher(teacher);
        
        // Tính tổng học sinh từ tất cả các phòng
        List<Classroom> classrooms = classroomRepository.findByTeacherAndIsActive(teacher, true);
        long totalStudents = classrooms.stream()
            .mapToLong(c -> classroomMemberRepository.countByClassroomAndStatus(c, ClassroomMember.Status.ACTIVE))
            .sum();
        ...
        if (activeSubscription != null) {
            ...
            if (daysRemaining <= 7 && daysRemaining > 0) {
                model.addAttribute("warningMessage", 
                    "Gói dịch vụ của bạn sẽ hết hạn trong " + daysRemaining + " ngày. Vui lòng gia hạn để tiếp tục sử dụng.");
            }
            
            // Check AI lesson usage
            Integer aiLessonsUsed = activeSubscription.getAiLessonsUsed() != null ? activeSubscription.getAiLessonsUsed() : 0;
            ...
            Integer testsCreated = activeSubscription.getTestsCreated() != null ? activeSubscription.getTestsCreated() : 0;
    ...
    public String questions(Authentication auth, Model model,
        ...
        try {
            ...
            System.out.println("=== DEBUG: Filters - subject: " + subject + ", grade: " + grade + ", difficulty: " + difficulty + ", keyword: " + keyword);
            ...
            List<Question> pageQuestions = (fromIndex < totalItems) ? 
                allQuestions.subList(fromIndex, toIndex) : new ArrayList<>();
    ...
    public String aiGenerateQuestions(@RequestParam String subject,
        ...
        for (Map<String, Object> qData : generatedQuestions) {
            ...
            if (qData.containsKey("optionA")) question.setOptionA((String) qData.get("optionA"));
            if (qData.containsKey("optionB")) question.setOptionB((String) qData.get("optionB"));
            if (qData.containsKey("optionC")) question.setOptionC((String) qData.get("optionC"));
            if (qData.containsKey("optionD")) question.setOptionD((String) qData.get("optionD"));
    ...
    public String aiGenerateLesson(@RequestParam String subject,
        ...
        if (!paymentService.canUseAILesson(teacher)) {
            model.addAttribute("errorMessage", "Bạn đã hết lượt tạo bài giảng AI trong tháng này. Vui lòng nâng cấp gói dịch vụ.");
            return "teacher/ai-generate-lesson";
        }
    ...
    public String aiGenerateTest(@RequestParam(required = false) String title,
        ...
        if (!paymentService.canUseAITest(teacher)) {
            model.addAttribute("errorMessage", "Bạn đã hết lượt tạo đề thi AI trong tháng này. Vui lòng nâng cấp gói dịch vụ.");
            return "teacher/ai-generate-test";
        }
    ...
    public String classrooms(Authentication auth, Model model) {
        ...
        for (Classroom classroom : classrooms) {
            long memberCount = classroomMemberRepository.countByClassroomAndStatus(
    ...
    public String viewClassroom(@PathVariable Long id, Authentication auth, Model model) {
        ...
        List<ClassroomMember> members = classroomMemberRepository.findByClassroomAndStatus(
            classroom, ClassroomMember.Status.ACTIVE
        );
    ...
    public String updateClassroom(@PathVariable Long id, @ModelAttribute Classroom classroomUpdate, 
        ...
        if (!classroom.getRoomId().equals(classroomUpdate.getRoomId())) {
            if (classroomRepository.existsByRoomId(classroomUpdate.getRoomId())) {
    ...
    public String removeStudentFromClassroom(@PathVariable Long classroomId, 
        ...
        Optional<ClassroomMember> memberOpt = classroomMemberRepository.findByClassroomAndStudent(classroom, student);
    ...
    private String generateRoomId() {
        ...
        } while (classroomRepository.existsByRoomId(roomId));
        return roomId;
    }
    ...
    private List<Question> generateAIQuestions(User teacher, String subject, String grade, 
        ...
        try {
            ...
            for (Map<String, Object> qData : aiQuestions) {
                ...
                if (qData.containsKey("optionA")) q.setOptionA((String) qData.get("optionA"));
                if (qData.containsKey("optionB")) q.setOptionB((String) qData.get("optionB"));
                if (qData.containsKey("optionC")) q.setOptionC((String) qData.get("optionC"));
                if (qData.containsKey("optionD")) q.setOptionD((String) qData.get("optionD"));
        ...
        if (plan.getPrice() == 0) {
            try {
                ...
                List<Subscription> activeSubscriptions = subscriptionRepository
                    .findByUserAndStatus(teacher, Subscription.Status.ACTIVE);
    ...
    public String aiHistory(Authentication auth, Model model) {
        User teacher = getCurrentUser(auth);
        List<AIHistory> histories = aiHistoryRepository.findByTeacherOrderByCreatedAtDesc(teacher);
        model.addAttribute("teacher", teacher);
